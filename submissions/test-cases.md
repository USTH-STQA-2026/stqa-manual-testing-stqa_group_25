...
### Nhóm 1: Đăng nhập & Đăng xuất (REQ-01)
...
### Nhóm 2: Xem, Tìm kiếm và Lọc sách (REQ-02, REQ-03)

| Mã TC | Mục tiêu kiểm thử | Tiền điều kiện | Bước thực hiện | Dữ liệu đầu vào | Kết quả mong đợi | REQ | Kỹ thuật |
|-------|-------------------|---------------|---------------|-----------------|------------------|-----|---------|
| **TC-07** | Hiển thị danh sách sách và trạng thái mặc định | Đã đăng nhập bằng `biet.hoang@email.com` | 1. Chuyển sang tab "Sách".<br>2. Quan sát danh sách sách. | (Không có) | Hiển thị đủ 20 đầu sách. BOOK003 hiển thị "Đang mượn", BOOK001 hiển thị "Có sẵn". | REQ-02 | EP |
| **TC-08** | Tìm kiếm sách bằng chữ thường/bình thường | Đang ở tab "Sách" | 1. Nhập từ khóa chữ thường vào ô tìm kiếm.<br>2. Nhấn "Tìm kiếm". | Từ khóa: `"lập trình"` | Hiển thị sách BOOK001 (Lập trình Flutter cơ bản) và sách BOOK009 (Nhập môn lập trình Python). | REQ-03 | EP |
| **TC-09** | Tìm kiếm sách bằng chữ toàn bộ IN HOA | Đang ở tab "Sách" | 1. Nhập từ khóa IN HOA vào ô tìm kiếm.<br>2. Nhấn "Tìm kiếm". | Từ khóa: `"FLUTTER"` | Hiển thị sách BOOK001 (Lập trình Flutter cơ bản). Không phân biệt hoa/thường. | REQ-03 | EP |
| **TC-10** | Tìm kiếm với khoảng trắng thừa (Leading/Trailing spaces) | Đang ở tab "Sách" | 1. Nhập từ khóa có nhiều dấu cách ở 2 đầu.<br>2. Nhấn "Tìm kiếm". | Từ khóa: `"   Flutter   "` | Hệ thống tự động cắt khoảng trắng (trim) và vẫn tìm ra sách "Lập trình Flutter cơ bản". | REQ-03 | BVA |
| **TC-11** | Tìm kiếm sách không có kết quả | Đang ở tab "Sách" | 1. Nhập từ khóa không tồn tại.<br>2. Nhấn "Tìm kiếm". | Từ khóa: `"Sách Nấu Ăn"` | Hiển thị thông báo "Không tìm thấy sách". | REQ-03 | EP |
| **TC-12** | Lọc sách theo thể loại đúng định dạng (Có viết hoa) | Đang ở tab "Sách" | 1. Nhập thể loại vào ô Lọc.<br>2. Quan sát kết quả. | Lọc: `"Công nghệ"` | Hiển thị danh sách các sách thuộc nhóm Công nghệ. | REQ-03 | EP |
| **TC-13** | Lọc sách theo thể loại không phân biệt hoa thường | Đang ở tab "Sách" | 1. Nhập thể loại chữ thường vào ô Lọc.<br>2. Quan sát kết quả. | Lọc: `"công nghệ"` | Hiển thị danh sách các sách thuộc nhóm Công nghệ. | REQ-03 | EP |
| **TC-14** | Lọc kết hợp Tìm kiếm không ra kết quả | Đang ở tab "Sách" | 1. Nhập tìm "Flutter".<br>2. Nhập lọc "Kinh tế". | Tìm: `"Flutter"`<br>Lọc: `"Kinh tế"` | Hiển thị "Không tìm thấy sách nào" do xung đột điều kiện. | REQ-03 | EP |

---

### Nhóm 3: Giao dịch Mượn & Trả sách (REQ-04, REQ-05)
...
### Nhóm 4: Quản lý của Thủ thư & Tra cứu (REQ-06, REQ-07, REQ-08)
